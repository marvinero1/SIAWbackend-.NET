...
namespace SIAW.Controllers.inventarios.modificacion
{
    ...
    [ApiController]
    public class docmodifinsolurgenteController : ControllerBase
    {
        ...
        private readonly string _controllerName = "docmodifinsolurgenteController";

        private readonly UserConnectionManager _userConnectionManager;
        public docmodifinsolurgenteController(UserConnectionManager userConnectionManager)
        {
            _userConnectionManager = userConnectionManager;
        }

        [HttpGet]
        [Route("getUltiSolUrgId/{userConn}")]
        public async Task<object> getUltiSolUrgId(string userConn)
        {
            try
            {
                ...
                {
                    var ultimoRegistro = await _context.insolurgente
        ...
        [HttpGet]
        [Route("obtSolUrgxModif/{userConn}/{id}/{nroId}/{codempresa}")]
        public async Task<object> obtSolUrgxModif(string userConn, string id, int nroId, string codempresa)
        {
            string conexionVPN = "";
            string idpf_almacen = "";
            int nroidpf_almacen = 0;
            try
            {
                string userConnectionString = _userConnectionManager.GetUserConnection(userConn);
                ...
                string codmoneda_total = "";

                insolurgente? cabecera = new insolurgente();
                List<detalleRequest> detalle = new List<detalleRequest>();

                using (var _context = DbContextFactory.Create(userConnectionString))
                {
                    cabecera = await _context.insolurgente.Where(i => i.id == id && i.numeroid == nroId).FirstOrDefaultAsync();

                    if (cabecera == null)
                    {
                        return BadRequest(new { resp = "No se encontró registro con los datos proporcionado." });
                    }
                    ...
                    detalle = await _context.insolurgente1.Where(i => i.codsolurgente == cabecera.codigo)
                        .Join(_context.initem,
                            p => p.coditem,
                            i => i.codigo,
                            (p, i) => new { p, i })
                        .Select(i => new detalleRequest
                        {
                            codsolurgente = i.p.codsolurgente,
                            coditem = i.p.coditem,
                            ...
                            saldoag = i.p.saldoag,
                            saldodest = i.p.saldodest,
                            stockmax = i.p.stockmax,
                            saldoarea = i.p.saldoarea,
                            precio = i.p.precio,
                            total = i.p.total,
                            cantidad_pedido = i.p.cantidad_pedido
                        }).ToListAsync();
                    ...
                    codmoneda_total = await Empresa.monedabase(_context, codempresa);

                    conexionVPN = await empaque_func.Getad_conexion_vpnFromDatabase_Sam(_context, cabecera.codalmdestino);

                    
                }
                using (var _context = DbContextFactory.Create(conexionVPN))
                {
                    try
                    {
                        var dtVPN = await _context.veproforma.Where(i => i.codigo == cabecera.codproforma_almacen).Select(i => new
                        {
                            i.codigo,
                            i.id,
                            i.numeroid,
                        ...
                        if (dtVPN != null)
                        {
                            idpf_almacen = dtVPN.id;
                            nroidpf_almacen = dtVPN.numeroid;
                        }
                    }
                    catch (Exception)
                    {
        ...
        public async Task<ActionResult<object>> grabarDocumento(string userConn, string codempresa, bool valida_aceptaSolUrg, bool solUrgcubreSaldoValido, bool solUrgVtaMaxValido, requestGabrarSolUrgente dataGrabar)
        {
            ...
            try
            {
                ...
                if (cabecera.codproforma_almacen != null)
                {
                    using (var _context = DbContextFactory.Create(conexion))
                    {
                        if (await ventas.proforma_transferida(_context, (int)cabecera.codproforma_almacen))
                        {
                            return StatusCode(203, new
                            {
                                valido = false,
                                resp = "Esta Proforma ya fue transferida, no puede ser modificada. Para modificarla anule el documento al cual fue transferida. ",
                                flete = cabecera.flete
                            });
                        }
                        if (await Ventas.proforma_anulada(_context, (int)cabecera.codproforma_almacen))
                        {
                            return StatusCode(203, new
                            {
                                valido = false,
                                resp = "Esta Proforma esta Anulada, no puede ser modificada. ",
                                flete = cabecera.flete
                            });
                        }
        ...
        private async Task<(bool valido, string msg)> validardatos(DBContext _context, insolurgente cabecera, List<insolurgente1> detalle)
        {
            ...
            bool docSolAnulada = await _context.insolurgente.Where(i => i.codigo == cabecera.codigo).Select(i => i.anulada).FirstOrDefaultAsync();
            if (docSolAnulada)
            {
                return (false, "Esta solicitud no se puede modificar porque esta Anulada.");
            }
            if (string.IsNullOrWhiteSpace(cabecera.id))
            {
                return (false, "No puede dejar el id de la sol. Urgente en blanco.");
            }
            if (cabecera.codproforma_almacen == null)
            {
                return (false, "El código de proforma de almacen esta como nulo.");
            }
